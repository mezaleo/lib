...
var CalendarEightysix = new Class({
   ...
   initialize: function(target, options) {
      ...
      //Extend date with unix timestamp parser and format
      Date.defineParser({
         re: /^[0-9]{10}$/,
         handler: function(bits) {
            return new Date.parse('Jan 01 1970').set('seconds', bits[0]);
         }
      });
      ...
      this.element = new Element('div', {
         'class': 'calendar-eightysix', 
         'html': innerHtml, 
         'style': 'display: '+ ((this.options.alwaysShow) ? 'block' : 'none')
      ...
      if(!this.options.disallowUserInput && this.options.linkWithInput && this.target.get('tag') == 'input') {
         this.target.addEvent('keyup', function() {
            var date = new Date().parse(this.target.get('value')).clearTime();
            if(date.isValid()) {
   ...
   renderYear: function() {
      ...
      for(var i = 1; i < 13; i++) {
         ...
         if(!(i % 4) && i != 12) row = new Element('div', {
            'class': 'row'
         }).injectBottom(this.currentContainer);
      }
   ...
   renderDecade: function() {
      ...
      for(var i = startYear; i < startYear + 12; i++) {
         ...
         if(!((i + 1) % 4) && i != startYear + 11) row = new Element('div', {
            'class': 'row'
         }).injectBottom(this.currentContainer);
      }
   ...
   slideLeft: function() {
      ...
      switch(this.view) {
         case 'month':
            this.viewDate.decrement('month');
            break;
         case 'year':
            this.viewDate.decrement('year');
            break;
         case 'decade':
            this.viewDate.set('year', this.viewDate.get('year') - 12);
            break;
      }
      this.render();
      ...
      .tween('left', [-this.currentContainer.getWidth(), 0]);
      this.tempContainer.set('tween', {
         'duration': this.options.slideDuration, 
         'transition': this.options.slideTransition
      })
      .tween('left', [0, this.tempContainer.getWidth()]);
   },
   ...
   slideRight: function() {
      ...
      switch(this.view) {
         case 'month':
            this.viewDate.increment('month');
            break;
         case 'year':
            this.viewDate.increment('year');
            break;
         case 'decade':
            this.viewDate.set('year', this.viewDate.get('year') + 12);
            break;
      ...
      .tween('left', [0, -this.currentContainer.getWidth()]);
   },
   ...
   switchContainers: function() {
      ...
      this.currentContainer.empty().removeClass('month').removeClass('year-decade').setStyles({
         'opacity': 1, 
         'display': 'block', 
         'z-index': 999
      });
   ...
   pick: function(e) {
      ...
      if($defined(e)) this.options.pickFunction(this.selectedDate);
   },
      ...
      format = format.replace(/%([a-z%])/gi,
         function($1, $2) {
            switch($2) {
               ...
               case 'n':
                  return date.get('mo') + 1;
               case 't':
                  return (date.getTime() / 1000).toInt();
            }
            return '%'+ $2;
         }
         );
