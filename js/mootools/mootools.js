...
(function() {
	...
	if (r) {
		r = [ "hasOwnProperty", "valueOf", "isPrototypeOf",
	...
	Array.from = function(i) {
		...
		return (k.isEnumerable(i) && typeof i != "string") ? (e(i) == "array") ? i
				: o.call(i)
				: [ i ];
	};
	...
	this.$each = function(w, v, x) {
		var i = e(w);
		((i == "arguments" || i == "collection" || i == "array" || i == "elements") ? Array
				: Object).each(w, v, x);
	};
		...
		.implement( {
			...
			flatten : function() {
				...
				for ( var b = 0, a = this.length; b < a; b++) {
					...
					d = d
							.concat((c == "array" || c == "collection"
									|| c == "arguments" || instanceOf(this[b],
									Array)) ? Array.flatten(this[b]) : this[b]);
				}
				return d;
			},
			pick : function() {
...
Function.implement( {
	...
	bind : function(c) {
		var a = this, b = (arguments.length > 1) ? Array.slice(arguments, 1)
				: null;
		return function() {
...
Function.implement( {
	create : function(b) {
		...
		return function(d) {
			var c = b.arguments;
			c = (c != null) ? Array.from(c) : Array.slice(arguments,
					(b.event) ? 1 : 0);
			if (b.event) {
...
(function() {
	...
	var a = navigator.userAgent.toLowerCase(), c = navigator.platform
			...
			|| [ null, "unknown", 0 ], g = k[1] == "ie" && l.documentMode;
	var p = this.Browser = {
	...
	String.implement("stripScripts", function(q) {
		var e = "";
		var r = this.replace(/<script[^>]*>([\s\S]*?)<\/script>/gi, function(s,
				t) {
			e += t + "\n";
			return "";
		});
	...
	try {
		...
		Array.from = function(q) {
			if (typeof q != "string" && Type.isEnumerable(q)
					&& typeOf(q) != "array") {
		...
		function(a, i) {
			...
			if (n.indexOf("key") != -1) {
				...
				if ((/click|mouse|menu/i).test(n)) {
					o = (!o.compatMode || o.compatMode == "CSS1Compat") ? o.html
							: o.body;
					m = {
						x : (a.pageX != null) ? a.pageX : a.clientX
								+ o.scrollLeft,
						y : (a.pageY != null) ? a.pageY : a.clientY
								+ o.scrollTop
					};

					c = {
						x : (a.pageX != null) ? a.pageX - i.pageXOffset
								: a.clientX,
						y : (a.pageY != null) ? a.pageY - i.pageYOffset
								: a.clientY
					};
					if ((/DOMMouseScroll|mousewheel/).test(n)) {
						l = (a.wheelDelta) ? a.wheelDelta / 120
								: -(a.detail || 0) / 3;
					}
					h = (a.which == 3) || (a.button == 2);
					if ((/over|out/).test(n)) {
						q = a.relatedTarget
								|| a[(n == "mouseover" ? "from" : "to")
										+ "Element"];
						var j = function() {
...
(function() {
	var a = this.Class = new Type("Class", function(h) {
		...
		var g = function() {
			...
			this.$caller = null;
			var i = (this.initialize) ? this.initialize.apply(this, arguments)
					: this;
	...
	var c = function() {
		...
		var g = this.$caller.$name, h = this.$caller.$owner.parent, i = (h) ? h.prototype[g]
				: null;
		if (!i) {
...
(function() {
	...
	j.search = function(U, z, H, s) {
		...
		if (typeof z == "string") {
					...
					if (u == "#") {
						...
						if (u == ".") {
							if (!this.isHTMLDocument
									|| ((!U.getElementsByClassName || this.brokenGEBCN) && U.querySelectorAll)) {
								break simpleSelectors;
							}
			...
			querySelector: if (U.querySelectorAll) {
				if (!this.isHTMLDocument
						...
						|| (this.brokenEmptyAttributeQSA && g.test(z))
						|| (!y && z.indexOf(",") > -1) || d.disableQSA) {
					break querySelector;
				}
				var S = z, x = U;
				if (!y) {
		...
		this.push = (!I && (s || (F.length == 1 && F.expressions[0].length == 1))) ? this.pushArray
				: this.pushUID;
		if (p == null) {
			p = [];
		}
...
var IFrame = new Type("IFrame", function() {
	...
	c.id = c.name = [ c.id, c.name,
			b ? (b.id || b.name) : "IFrame_" + String.uniqueID() ].pick();
	...
	Element
			.implement( {
				...
				getProperty : function(q) {
					q = o[q] || q;
					var p = g[q] || m[q];
					return (p) ? this[p] : (d[q]) ? !!this[q]
							: (l.test(q) ? this.getAttribute(q, 2) : (p = this
									.getAttributeNode(q)) ? p.nodeValue : null)
									|| null;
				},
							...
							.each(
									function(r) {
										...
										var s = (r.get("tag") == "select") ? r
												...
												: ((q == "radio" || q == "checkbox") && !r.checked) ? null
														: r.get("value");
										Array
												.from(s)
												.each(
														function(t) {
															if (typeof t != "undefined") {
																p
																		.push(encodeURIComponent(r.name)
																				+ "="
...
(function() {
	...
	Element.implement( {
		getComputedStyle : function(h) {
			...
			return (f) ? f.getPropertyValue((h == a) ? "float" : h.hyphenate())
					: null;
		},
		...
		getStyle : function(l) {
			...
			if (Browser.opera || (Browser.ie && isNaN(parseFloat(f)))) {
				if ((/^(height|width)$/).test(l)) {
					var g = (l == "width") ? [ "left", "right" ] : [ "top",
							"bottom" ], i = 0;
					g.each(function(m) {
						i += this.getStyle("border-" + m + "-width").toInt()
...
(function() {
	...
	[ Element, Window, Document ].invoke("implement", {
		...
		removeEvent : function(e, d) {
			...
			return (Element.NativeEvents[e]) ? this.removeListener(e, g,
					arguments[2]) : this;
		},
		addEvents : function(b) {
			for ( var c in b) {
...
(function() {
			...
			.implement( {
				...
				getOffsets : function() {
					if (this.getBoundingClientRect && !Browser.Platform.ios) {
						var r = this.getBoundingClientRect(), o = document
								.id(this.getDocument().documentElement), q = o
								.getScroll(), t = this.getScrolls(), s = (k(
								this, "position") == "fixed");
						return {
							x : r.left.toInt() + t.x + ((s) ? 0 : q.x)
									- o.clientLeft,
							y : r.top.toInt() + t.y + ((s) ? 0 : q.y)
									- o.clientTop
						};
	...
	function f(m) {
		var n = m.getDocument();
		return (!n.compatMode || n.compatMode == "CSS1Compat") ? n.html
				: n.body;
	}
})();
Element.alias( {
	...
	var c = this.Request = new Class(
			{
				...
				send : function(o) {
					...
					var j = o.data, f = String(o.url), e = o.method
							.toLowerCase();
					switch (typeOf(j)) {
					case "element":
					...
					if (this.options.urlEncoded
							&& [ "post", "put" ].contains(e)) {
						var g = (this.options.encoding) ? "; charset="
					...
					n.open(e.toUpperCase(), f, this.options.async,
